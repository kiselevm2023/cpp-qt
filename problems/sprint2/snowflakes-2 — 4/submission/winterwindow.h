...
public:
    ...
    void Draw(prac::QPainter& painter) const {
        ...
        // Устанавливаем перо с цветом и толщиной линии
        QPen pen(params_.color, params_.line_width);
        painter.setPen(pen);

        // Рисуем 6 лучей
        for (int i = 0; i < 6; ++i) {
            double angle = params_.rotation + 60 * i; // Вычисляем угол
            DrawRotatedVector(painter, params_.center, angle, params_.size); // Рисуем каждый луч
        }
    }
