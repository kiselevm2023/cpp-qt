...
void MainWindow::on_btn_dot_clicked()
{
    ...
    if (!input_number_.contains(kDot) && input_number_.length() < kMaxDigits) { // Добавлена проверка на количество цифр
        input_number_ += kDot;
        ui->l_result->setText(input_number_);
    }
}
...
void MainWindow::ProcessNumber(Number n) {
    ...
    if (input_number_.length() < kMaxDigits) { // Добавлена проверка на количество цифр
